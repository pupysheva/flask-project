...
pred = svd.predict(train_data)
print(datetime.now(), 'finish svd.predict. Start mean and sqrt')
mae = mean_absolute_error(train_data["rating"], pred)
mse = mean_squared_error(train_data["rating"], pred)
rmse = np.sqrt(mse)
...
pred_trivial = df_means_ratings_for_movies.merge(train_data.reset_index(drop=True), how='inner',
                                                 left_on='i_id',  right_on='i_id')
print("df_means_ratings_for_movies", df_means_ratings_for_movies)
print("pred_trivial", pred_trivial)
